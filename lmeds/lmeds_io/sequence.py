...
class TestSetupError(Exception):
    ...
    def __str__(self):
        txtString = (
            "ERROR: The following keys were found in "
...
class InvalidFirstLine(Exception):
    ...
    def __str__(self):
        return (
            "ERROR: The first line in a sequence file must be "
...
class InvalidSequenceFileError(Exception):
    ...
    def __str__(self):
        return (
            "ERROR: The first command in a sequence file cannot be "
...
class UserSequencePathError(Exception):
    ...
    def __str__(self):
        return (
            "Path '%s' does not exist in test folder. "
            "Please create it and try again." % self.path
        )
    ...
    def __init__(self, webSurvey, sequenceFN, individualSequenceName=None):
        ...
        if individualSequenceName is not None:
            ...
            if not os.path.exists(newPath):
                raise UserSequencePathError(
                    join("<test_root>", "individual_sequences", sequenceTitle)
                )
...
def recChunkLine(line, splitItem=None):
    ...
    while True:
        ...
        try:
            bracketStartIndex, endIndex = _parse(line, char1, char2, startIndex)
        except ValueError:
            break
