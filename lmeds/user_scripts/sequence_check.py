...
def checkSequenceFile(survey):
    ...
    for pageNum in range(seq.getNumPages()):
        ...
        try:
            ...
            continue
        except (utils.FilesDoNotExist, loader.TextNotInDictionaryException) as e:
            print("Page %d: %s\n" % (pageNum, str(e)))
            numErrors += 1
            continue
        ...
        try:
            ...
            errStr = (
                "Page %d: Problem with at least one of the arguments: " "'%s'\n%s:%s\n"
            )
            print(
                errStr
                % (
                    pageNum,
                    str(seq.testItemList[pageNum]),
...
if __name__ == "__main__":
    ...
    parser.add_argument("sequence_fn", action="store", help=("The sequence .txt file"))
    parser.add_argument(
        "language_fn", action="store", help=("The language dictionary .txt file")
    )
    parser.add_argument(
        "disable_refresh_flag",
        action="store",
        help=("Disable the refresh button?  true or false?"),
    )
    audioMsg = (
        'The audio file types used.  Separate with ","and no spaces.'
        "eg:.mp3,.ogg,.wav.  Default=.mp3,.ogg"
    )
    parser.add_argument("audio_ext_list", action="store", help=audioMsg)
    ...
    try:
        ...
        _language_fn = raw_input(("Enter the name of the language " "dictionary:\n"))
        _disable_refresh_flag = raw_input(
            ("Disable the refresh button?  " "true or false?\n")
        )
        audioPrompt = "Audio files to use (use commas, no spaces " "eg:.mp3,.wav)\n"
