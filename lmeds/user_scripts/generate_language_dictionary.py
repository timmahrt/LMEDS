...
def generateLanguageDictionary(mode, testName, sequenceFN, outputFN):
    ...
    webSurvey = lmeds_main.WebSurvey(
        testName, sequenceFN, None, disableRefreshFlag=True, sourceCGIFN=None
    )

    outputFNFullPath = join(webSurvey.surveyRoot, outputFN)

    keepTextStrings = mode in [
        "update",
        "crop",
    ]
    oldDictionary = None
    ...
    implicitDict = {
        core_pages.MediaTestPage.pageName: core_pages.MediaTestEndPage.pageName,
        core_pages.ConsentPage.pageName: core_pages.ConsentEndPage.pageName,
        core_pages.LoginPage.pageName: core_pages.LoginErrorPage.pageName,
    }
    for pageName in pageNameList:
        ...
        if pageName in implicitDict.keys():
            page = factories.loadPage(webSurvey, implicitDict[pageName], args, kargs)
    ...
    comboPages = [pageTuple for pageTuple in invKeyDict.keys() if len(pageTuple) > 1]
    ...
    if mode == "new":
        try:
            ...
            errorStr = (
                "Dictionary file '%s' already exists.  Please delete "
    ...
    if outputFlag is True:
        if os.path.exists(outputFNFullPath):
            backupFN = os.path.splitext(outputFNFullPath)[0] + "_autobackup.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            "'update' will add any new textkeys used in the "
    ...
    parser.add_argument(
        "test_name",
        action="store",
        help=("The name of the root folder that stores the " "test files"),
    )
    parser.add_argument(
        "sequence_fn",
        action="store",
        help=("The sequence .txt file to extract textkeys " "from"),
    )
    parser.add_argument(
        "output_fn",
        action="store",
        help=("The dictionary .txt to output textkeys and " "their value strings"),
    )
    ...
    try:
        ...
        _test_name = raw_input(("Enter the name of your test " "(the root folder):\n"))
        _sequence_fn = raw_input("Enter the name of your sequence file:\n")
        _output_fn = raw_input(
            ("Enter the name of the dictionary file to " "write to:\n")
        )
    else:
        _mode = cmdArgs.mode
