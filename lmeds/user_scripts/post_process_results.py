...
class EmptyUserDataFile(Exception):
    ...
    def __str__(self):
        errMsg = (
            "User results file '%s' contains no user data. "
...
def postProcessResults(testName, sequenceFN, removeDuplicatesFlag, removeItemList=None):
    ...
    if not all([len(response) == testLen for response in userResponseList]):
        print("ERROR: Not all responses in folder %s are the same length" % pathToData)
        countDict = {}
    ...
    if "survey" in uniquePageList:
        ...
        transpose_survey.transposeSurvey(
            join(pathToData, "survey"), surveyNameList, outputPath
        )
    ...
    for pageName in prominencePageList:
        if pageName in uniquePageList:
            transpose_rpt.transposeRPT(
                join(pathToData, pageName), txtPath, pageName, outputPath
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="*",
        help=(
            "A list of stimuli names to remove from "
    ...
    try:
        ...
        _remove_duplicates_flag = raw_input("Remove duplicate items? " "(true/false)\n")
        _remove_items_flag = raw_input(
            "Would you like to remove any items "
            "(e.g. sample/test items)? (yes/no):\n"
        )
        if _remove_items_flag.lower() == "yes":
            _remove_item_list = raw_input(
