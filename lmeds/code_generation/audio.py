...
def generateEmbed(wavDir, nameList, extList, audioOrVideo):
    ...
    for name in nameList:
        ...
        if any([not os.path.exists(join(wavDir, fn)) for fn in fnList]):
            raise utils.FilesDoNotExist(wavDir, fnList, True)

    nameSet = set(nameList)
    nameList = [
        "'%s'" % os.path.splitext(os.path.split(name)[1])[0] for name in nameSet
    ]
...
def generateAudioButton(
    name, idNum, buttonLabel, pauseDurationSec=0, example=False, autoSubmit=False
):
