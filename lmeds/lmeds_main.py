...
TXT_KEY_LIST = ["continue_button", "metadata_description", "back_button_warning"]
...
class WebSurvey(object):
    ...
    def run(self, cgiForm=None):
        ...
        self.buildPage(
            pageNum, cookieTracker, page, userName, self.testSequence, self.sourceCGIFN
        )
    ...
    def runDebug(self, page, pageNum=1, cookieTracker=True, userName="testing"):
        ...
        self.buildPage(
            pageNum, cookieTracker, page, userName, testSequence, self.sourceCGIFN
        )
    ...
    def buildPage(
        self, pageNum, cookieTracker, page, userName, testSequence, sourceCGIFN
    ):
        ...
        validationTemplate = (
            "// Validate page form before submitting\n"
        ...
        if "embed" in updateDict.keys():
            ...
            if "audioLoader" in updateDict["embed"]:
                ...
                try:
                    minMaxTxt += (
                        "audioLoader.numAudioButtons = %s;\n" % page.numAudioButtons
                    )
                except AttributeError:
                    pass

                minMaxTxt += (
                    "audioLoader.numUniqueSoundFiles = "
    ...
    def _testSequenceOverride(self, userName):
        ...
        if self.individualSequences is True:
            if userName != "" and self.sequenceFN != userName + ".txt":
                testSequence = sequence.TestSequence(self, self.sequenceFN, userName)
