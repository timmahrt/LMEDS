...
def _getScores(userData, scoreType):
    ...
    scoreList = [row for row in scoreList if all([item != "" for item in row])]

    sumList = [
        "%.03f" % (sum([float(val) for val in subList]) / len(subList))
...
def _outputScores(featPath, aspect, stimulusID, returnDict, sumList, scoreType):
    ...
    with io.open(fn, "w", encoding="utf-8") as fd:
        fd.write(
            "\n".join([",".join(val) for val in returnDict[stimulusID][scoreType]])
        )
...
def _buildHeader(fnList, pageName, doSequenceOrder, sampleHeader):
    ...
    anonBNameList = [userNameTemplate % (i + 1, "b") for i in range(len(fnList))]

    pNameList = [os.path.splitext(name)[0] + ".p" for name in fnList]
    anonPNameList = [userNameTemplate % (i + 1, "p") for i in range(len(fnList))]
    headerDict = {
        "boundary": (bNameList, anonBNameList),
    ...
    headerList.extend([name for name in nameList])
    anonHeaderList.extend([name for name in anonNameList])
    ...
    if doSequenceOrder:
        ...
        tmpTuple = transpose_utils.getUserSeqHeader(fnList, sequencePageCode, oom)
...
def _unifyRow(row):
    return [",".join(cells) if isinstance(cells, list) else cells for cells in row]
...
def transposeRPT(path, txtPath, pageName, outputPath):
    ...
    if len(orderListOfLists) > 0:
        orderList = [",".join(row) for row in utils.safeZip(orderListOfLists, True)]
    ...
    headerRow, anonHeaderRow = _buildHeader(
        fnList, pageName, doUserSeqHeader, idKeyList[0]
    )
