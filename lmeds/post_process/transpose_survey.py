...
def transposeSurvey(path, surveyFullPathList, outputPath):
    ...
    for stimulusID in idKeyList:
        returnDict[stimulusID] = [
            list(subTuple)
            for subTuple in utils.safeZip(returnDict[stimulusID], enforceLength=True)
        ]
        ...
        for row in returnDict[stimulusID]:
            ...
            try:
                total = str(sum([int(val) if val != "" else 0 for val in row[2:]]))
            except ValueError:
                total = "-"
            newData.append(
                row[:2]
        ...
        mainSurveyData = [",".join(subList) for subList in returnDict[stimulusID]]
