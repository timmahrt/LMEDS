...
class SurveyPage(abstract_pages.NonValidatingPage):
    ...
    def __init__(self, surveyName, *args, **kargs):
        ...
        self.surveyItemList = survey.parseSurveyFile(
            join(self.surveyRoot, self.surveyFN)
        )
                ...
                if not value:
                    ...
                    if itemType == "Multiline_Textbox":
                        ...
                        if isinstance(value, list):
                            value = [utils.decodeUnicode(subval) for subval in value]
                        else:
                            value = [
                        ...
                        valueList = [
                            "1" if i in indexList else "0" for i in range(len(argList))
                        ]
                        value = ",".join(replaceCommas(valueList))
...
class MediaChoicePage(abstract_pages.AbstractPage):
    ...
    def __init__(
        ...
        mediaListOfLists,
        responseButtonList,
        mediaButtonLabelList=None,
        transcriptList=None,
        bindPlayKeyIDList=None,
        bindResponseKeyIDList=None,
        timeout=None,
        *args,
        **kargs
    ):
        ...
        self.nonstandardSubmitProcessList = [("widget", "media_choice")]
    ...
    def _getHTMLTxt(self):
        ...
        return htmlTxt % (labelRow, buttonRow)
    ...
    def getHTML(self):
        ...
        for i in range(len(self.mediaList)):
            ...
            mediaButtonHTML = audio.generateAudioButton(
                self.mediaList[i], i, audioLabel, self.pauseDuration, False
            )
        ...
        if self.transcriptList is not None:
            transcriptList = [
                loader.loadTxtFile(join(self.txtDir, transcript + ".txt"))
                for transcript in self.transcriptList
            ]
            transcriptList = [
                "<td>%s</td>" % "<br />".join(transcript)
        ...
        mediaNames = [
            mediaName for mediaSubList in self.mediaList for mediaName in mediaSubList
        ]
        if self._doPlayMedia():
            ...
            embedTxt += "\n\n" + audio.generateEmbed(
                self.wavDir, list(set(mediaNames)), extList, self.audioOrVideo
            )
...
class MediaSliderPage(abstract_pages.AbstractPage):
    ...
    def _getHTMLTxt(self):
        ...
        sliderHTML = (
            '%(leftTxt)s<input type="range" name="media_slider" '
            'min="%(min)s" max="%(max)s" id="%(id)s" disabled />'
            "%(rightTxt)s<br />%(label)s <br />"
        )
        sliderHTML %= {
            "min": str(self.sliderMin),
    ...
    def getHTML(self):
        ...
        audioLabel = self.textDict["play_button"]
        playBtnSnippet = audio.generateAudioButton(
            self.mediaName, 0, audioLabel, 0, False
        )
...
class MediaListPage(abstract_pages.AbstractPage):
    ...
    def __init__(
        self, audioOrVideo, pauseDuration, minPlays, maxPlays, mediaList, *args, **kargs
    ):
    ...
    def getHTML(self):
        ...
        audioLabel = self.textDict["play_button"]
        htmlText %= (
            audio.generateAudioButton(
                self.mediaList, 0, audioLabel, self.pauseDuration, False, True
            )
            + "<br />"
        )
        ...
        if self.audioOrVideo == "audio":
            ...
            extList = self.webSurvey.videoExtList
        embedTxt += "\n\n" + audio.generateEmbed(
            self.wavDir, list(set(self.mediaList)), extList, self.audioOrVideo
        )
