...
Core pages may appear in any LMEDS tests, regardless of what kind of experiment
it is.  Pages that provide information to users, get their user name or consent,
error pages, etc.
'''
...
class TextPage(abstractPages.NonValidatingPage):
    ...
    </div><br />''' % (loader.getText('title'), loader.getText(self.textName))
...
class TextAndAudioPage(abstractPages.NonValidatingPage):
    ...
    def getHTML(self):
        ...
        audioButtonList = [audio.generateAudioButton(name, i, 0, False) for i, name in enumerate(audioNameList)]
