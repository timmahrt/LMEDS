...
class LoginPage(abstract_pages.NonRecordingPage):
    ...
    def _getHTMLTxt(self):
        ...
        productNote %= (constants.softwareLogo, constants.softwareName)
    ...
    def getHTML(self):
        ...
        pageTemplate = join(self.webSurvey.htmlDir, "blankPageWValidation.html")
...
class LoginErrorPage(LoginPage):
    ...
    def _getHTMLTxt(self):
        ...
        if "%s" not in userNameErrorTxt:
            ...
            errorMsg = (
                "Please add a '%s' for the user name in the text "
                "associated with this key"
            )
            raise loader.BadlyFormattedTextError(
                errorMsg, textKey, self.webSurvey.textDict
            )
...
class TextAndMediaPage(abstract_pages.NonValidatingPage):
    ...
    def getHTML(self):
        ...
        embedTxt += self._getKeyPressEmbed()
        embedTxt += "\n\n" + audio.generateEmbed(
            self.wavDir, mediaNameList, extList, self.audioOrVideo
        )
...
class MediaTestPage(abstract_pages.NonRecordingPage):
    ...
    def __init__(self, audioOrVideo, mediaName, *args, **kargs):
        ...
        self.processSubmitList = ["validateForm()", "audioLoader.verifyAudioPlayed()"]
    ...
    def getHTML(self):
        ...
        audioLabel = self.textDict["play_button"]
        htmlText %= audio.generateAudioButton(self.mediaName, 0, audioLabel, 0, False)
        htmlText += "<br />"
