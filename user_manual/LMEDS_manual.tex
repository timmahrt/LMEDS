...
\textbf{Development of LMEDS was supported by NSF BCS 12-51343 to Jennifer Cole and JosÈ I. Hualde.}
...
\paragraph{}
Each surveyÄô requires its own survey file (e.g. presurvey.txt, postsurvey.txt).
...
\paragraph{}
A section is denoted with ``-''Äò characters on the line above and below the section name.  The lengths of these lines (the number of ``-'') doesn't matter but should be kept the same length to be visually more coherent.  
...
\paragraph{}
A key is denoted in the same way with ``Äò=''Äô characters.  Keys must be the same, regardless of the language. 

\subsubsection{Texts}

\paragraph{}
A Äò``text''Äô appears after a key and before another key or section.  Unlike sections and keys, text is undecorated text.  
...
\paragraph{}
The section denotes that these keys are used in the text pages, which tend to be used for giving instructions.  Whenever LMEDS needs to display the title, it will look at the text for the key ``Äòtitle'' Äîin this case ``Perception of Spoken Discourse''.

%%%%%%%%%%%%%%%%%%%%%
\subsection{SurveyÄô file specification}
...
\paragraph{}
Thus we have a survey with (ignoring the first item, which doesn't take user input) 5 items in total (2 being subitems).  The third item shows how multiple data entry fields can be placed on a single question (if the user selects ``Äòother''Äô for the first question they are expected to specify what they meant in the Textbox).
...
\paragraph{}
The sequence file contains the items that will be presented in a test.  The first line in a test is the test name which should be prefixed with a ``Äò*''Äô (e.g. *My\_Test\_Sequence) (\textbf{data from the experiment will be output to a folder with the test name.  Giving multiple sequence files the same name, means that they will dump their output to the same folder}).  The second item must be ``login''Äô where users create a name to associate their data with.  If login is not the item on the second line, unexpected behavior can result.
Subsequent items are presented in a linear fashion as users progress through the sequence.  The last item in the sequence must be ``end''Äô.
...
\paragraph{}
This second way is useful, for example, if you wanted to specify the value for a later variable such as ``presentAudio'' but not ``instructions''Äô.  The only way to do this is by naming the variable 
...
\paragraph{}
If one attempted the same list of arguments but without referring to the variable name, this would set ``instructions=true'' which would cause an error unless instructions named ``Äòtrue'' existed in the dictionary file.
...
\paragraph{}
``survey''Äô takes a single argument, the file name of the survey that it should load
...
\paragraph{}
This would load the survey stored in the file called ``Äòpresurvey.txt''
...
\paragraph{}
Users are presented with an audio file and the associated transcript.  They can click on a word to indicate that it is prominent.  This changes the selected word to \textbf{red}.  They can click it again to change it back to blackÄô.
...
\item maxPlays -Äì the maximum number of times the audio file can be played before the audio button is disabled. 
...
\item presentAudio -Äì either ``true'' or ``false'', specifies whether the audio is hidden or presented.  If ``false''Äô the values for ``ÄòminPlays''Äô and ``ÄòmaxPlays''Äô are ignored. \textbf{Default}: True
\end{itemize}
...
\paragraph{}
``Äòboundary'' takes the same arguments as \textbf{prominence} pages but with the addition of one final, optional argument:
...
\paragraph{}
A combination of prominences and boundaries.  Users first mark boundaries.  They then can mark prominences.  While marking prominences they can see but not change the boundaries that they marked--Äîthis is the only difference between this page and splitting the prominence and boundary task across two pages.
...
\subsubsection{same\_different\_stream}

\paragraph{}
The user is presented a series of audio files and has to decide whether they are the same or different.

\paragraph{}
The arguments for same\_different\_stream are the same as for audio\_list
...
\paragraph{}
``axb'' takes the following arguments

\begin{itemize}
\item the audio file for X
\item the audio file for A
\item the audio file for B
\item the minimum number of plays
\item the maximum number of plays
\end{itemize}
...
The user hears one audio file and decides if it is an example of text prompt A or B.

\paragraph{}
``ab''Äô takes the following arguments

\begin{itemize}
\item name of the audio file
\item the minimum number of plays
\item the maximum number of plays
\end{itemize}
...
\subsubsection{axb, same\_different, ab variants}

There are a number of variants of these three experimental paradigms available in LMEDS that are not documented here.  For more options, please see \textbf{/lmeds/pages/axb\_pages.py}.  In the future, these variants will be unified into a broader, more flexible single interface.
...
\paragraph{}
For most needs, the page type and user response are the most important bits of information.  The page type is separated from the rest of the row by the occurrence of the first comma.  The user input is separated from the rest of the row by the sequence ``Äò;,''.  The argument list is enclosed in \texttt{[ ]} and separated from the peripheral information by a comma.  
